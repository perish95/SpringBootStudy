...
class Ex05OneToManyTest {
	...
	void oneToManyTest() {
		// 저장(부모 엔티티 먼저 생성)
		Post post = new Post("LOSTARK");

		// 비지니스 메서드(편의상 만든 메서드)로 자식 엔티티 생성 및 연결
		PostComment comment1 = new PostComment("NEVER DIE");
		PostComment comment2 = new PostComment("NAVER PAY");
		PostComment comment3 = new PostComment("DIE");
		...
		// new PostComment("NEVER DIE")
		// insert into post_comments (content) values (?)

		// [중요: 일대다 단방향의 특징]
		// 부모엔티티가 자식 엔티티의 FK값을 바꾸기 위해 UPDATE 쿼리를 추가로 날림
